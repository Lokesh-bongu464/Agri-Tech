...
const Alogin = ({ showNotification }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const result = await login(email, password, "admin"); // Call login from context for admin
    if (result.success) {
      showNotification(
        result.message || "Logged in successfully as Admin!",
        "success"
      );
